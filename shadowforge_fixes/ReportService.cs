...
///
/// HOW IT WORKS:
///   GenerateReportAsync()  → assembles SimulationReport from session data
///   ExportToPdfAsync()     → renders HTML template → PDF bytes via PuppeteerSharp
///   ExportToJsonAsync()    → serialises report to indented JSON
/// </summary>
public sealed class ReportService : IReportService
{
    ...
    // ── IReportService: build the report object ───────────────────────────────
      ...
      <div class=""event-row"" style=""border-left:3px solid {ThreatColor(evt.ThreatLevel)};padding-left:10px"">
        ...
        <div class=""event-ts"">{evt.Timestamp:HH:mm:ss}</div>
                    ...
                    if (mod.Events.Count > 15)
                        sb.Append($@"<p style=""font-size:11px;color:#94a3b8;margin-top:10px"">… and {mod.Events.Count - 15} more events not shown.</p>");
                    sb.Append("</div>");
                }
...
<div class=""page"" style=""page-break-after:auto"">
  <div class=""report-footer"">
    <span>ShadowForge APT Emulation Framework — CONFIDENTIAL</span>
    ...
    // ── PuppeteerSharp PDF renderer ───────────────────────────────────────────

    private async Task<byte[]> RenderHtmlToPdfAsync(string html)
    {
        // PuppeteerSharp renders the HTML via headless Chromium → PDF bytes.
        // It honours @page CSS (A4, no margins) set in the template above.

        await PuppeteerSharp.Puppeteer.FetchAsync(product: PuppeteerSharp.BrowserTag.Latest,
            downloadPath: Path.Combine(Path.GetTempPath(), "sf_chromium"));
        ...
        {
            Format             = PuppeteerSharp.PaperFormat.A4,
            PrintBackground    = true,
            MarginOptions      = new PuppeteerSharp.MarginOptions { Top = "0", Bottom = "0", Left = "0", Right = "0" }
        });
